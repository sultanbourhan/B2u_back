...
exports.create_company_advertisements_my =asyncHandler( async (req, res, next)=>{
    ...
    if (advertisement.length > 0) {
                return next(new ApiError(`.لا يمكنك إضافة إعلان آخر خلال 24 ساعة من الإعلان الأخير`, 403));
            }
...
exports.get_Categorey = asyncHandler(async (req, res, next) =>{
    ...
    res.status(200).json({data : Categorey , result:Categorey.length})
})
...
exports.create_Company_requests = asyncHandler(async (req, res, next) =>{
    ...
    if(company){
        return next(new ApiError(`لديك حساب شركة ولا يمكنك فتح حساب آخر.`, 404))
    }

    const Company_ = await Company_requestsmodel.findOne({user : req.user._id})

    if(Company_){
        return next(new ApiError(`لديك طلب للحصول على حساب شركة ولا يمكنك فتح حساب آخر.`, 404))
    }
...
exports.get_Company_requests = asyncHandler(async (req, res, next) =>{
    ...
    const Company_requests = await Company_requestsmodel.find().populate({path:"user"}).populate({path:"categorey"})
